...
class BaleClient
{
    private const API_BASE = 'https://tapi.bale.ai/bot';
    ...
    {
        $this->token = trim((string)getSetting('bale_bot_token', ''));
        $this->channel = trim((string)getSetting('bale_channel_id', ''));
    }
    ...
    public function sendProduct(array $product): array
    {
        ...
        if ($imageUrl !== '') {
            $tmp = tempnam(sys_get_temp_dir(), 'bale_');
            if ($tmp !== false) {
    ...
    private function request(string $method, array $fields, bool $multipart = false): array
    {
        if (!$this->isConfigured()) {
            return ['ok' => false, 'error' => 'Bale is not configured'];
        }
        ...
        if (empty($decoded['ok']) && !isset($decoded['error'])) {
            $decoded['error'] = (string)($decoded['description'] ?? 'Bale API error');
        }
