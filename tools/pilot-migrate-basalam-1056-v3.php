...
function normTitle3(string $title): string {
...
function restoreMaps3(PDO $db, array $productMap, array $variationMaps, int $wcId): void {
    $db->beginTransaction();
    try {
...
function setVariantSku3(BasalamClient $client, int $productId, int $variantId, ?string $sku): void {
    $payloads = $sku === null || $sku === '' ? [['sku'=>null], ['sku'=>'']] : [['sku'=>$sku]];
    $lastError = null;
    foreach ($payloads as $payload) {
        $r = $client->updateProductVariation($productId, $variantId, $payload);
        if (!$r['error']) return;
        $lastError = $r['error'];
    }
    throw new RuntimeException('Variant SKU update failed for #'.$variantId.': '.$lastError);
}
...
$oldStatus = (int)($old['status']['value'] ?? $old['status'] ?? 0);
$oldShowable = (bool)($old['is_showable'] ?? false);
if ($oldCat === $expectedCat) throw new RuntimeException('Old product is already in correct category.');
if ($oldStatus !== 3790 || $oldShowable) throw new RuntimeException('Legacy product is not safely unpublished.');
if (normTitle3($oldTitle) === normTitle3($wooTitle)) throw new RuntimeException('Legacy title still conflicts with Woo title.');

// Ensure there is no already-created correct-category product with the Woo title.
...
if (!$search['error']) {
    ...
    if (is_array($items)) {
        foreach ($items as $item) {
            ...
            if (normTitle3($name) !== normTitle3($wooTitle)) continue;
...
foreach ((array)($old['variants'] ?? $old['variant'] ?? []) as $variant) {
    ...
    if ($vid <= 0) continue;
    $oldVariantSkus[$vid] = trim((string)($variant['sku'] ?? ''));
}

$newId = 0;
$mapsChanged = false;
$legacySkusReleased = false;
try {
    // Release any SKU held by the hidden legacy variants before creating the replacement.
    foreach ($oldVariantSkus as $vid => $sku) {
        if ($sku !== '') setVariantSku3($client, $oldId, $vid, null);
    }
    $legacySkusReleased = true;

    $verifyOld = $client->getProduct($oldId, true);
    if ($verifyOld['error']) throw new RuntimeException('Legacy verification failed: '.$verifyOld['error']);
    foreach ((array)($verifyOld['body']['variants'] ?? $verifyOld['body']['variant'] ?? []) as $variant) {
        if (trim((string)($variant['sku'] ?? '')) !== '') {
            throw new RuntimeException('Legacy variant SKU was not released; refusing create.');
        }
    }

    echo 'MIGRATE_BEFORE=' . json_encode([
        ...
        'old_title'=>$oldTitle,'old_status'=>$old['status'] ?? null,
        'released_variant_skus'=>array_keys(array_filter($oldVariantSkus, fn($x)=>$x!=='')),
        'woo_variations'=>count($variations),
    ], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) . PHP_EOL;

    $buildRef = new ReflectionMethod(BasalamSync::class, 'buildProductPayload');
    $buildRef->setAccessible(true);
    $payload = $buildRef->invoke($sync, $product, $variations, $expectedCat, true);
    ...
    $newRead = $client->getProduct($newId, true);
    if ($newRead['error']) throw new RuntimeException('New product read failed: '.$newRead['error']);
    $new = (array)$newRead['body'];
    ...
    if (normTitle3((string)($new['name'] ?? $new['title'] ?? '')) !== normTitle3($wooTitle)) {
        throw new RuntimeException('New product title mismatch.');
    }

    // Move only this Woo mapping to the replacement product, preserving rollback data.
    $db->beginTransaction();
    try {
        $db->prepare('DELETE FROM basalam_variation_map WHERE wc_product_id=:wc')->execute(['wc'=>$wcId]);
        $u = $db->prepare(
    ...
    $capture = $captureRef->invoke($sync, $wcId, $newId, $variations, (array)($createBody['variants'] ?? []));
    ...
    if ($finalRead['error']) throw new RuntimeException('Final product read failed: '.$finalRead['error']);
    ...
    echo 'MIGRATE_SUCCESS=' . json_encode([
        ...
        'final_showable'=>$final['is_showable'] ?? null,
        'final_available'=>$final['is_available'] ?? null,
        'final_sku'=>$final['sku'] ?? null,
        'final_variants'=>$final['variants'] ?? $final['variant'] ?? [],
        'final_map'=>$finalMap,
        'variation_capture'=>$capture,
        'sync_warnings'=>$syncResult['warnings'] ?? [],
        'image_warnings'=>$warnings,
    ], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) . PHP_EOL;
} catch (Throwable $e) {
    if ($mapsChanged) restoreMaps3($db, $oldMap, $oldVariationMaps, $wcId);

    if ($newId > 0) {
        $nr = $client->getProduct($newId, true);
        if (!$nr['error']) {
            foreach ((array)($nr['body']['variants'] ?? $nr['body']['variant'] ?? []) as $variant) {
                $vid = (int)($variant['id'] ?? 0);
                if ($vid > 0 && trim((string)($variant['sku'] ?? '')) !== '') {
                    try { setVariantSku3($client, $newId, $vid, null); } catch (Throwable $ignored) {}
                }
            }
        }
    ...
    if ($legacySkusReleased) {
        foreach ($oldVariantSkus as $vid => $sku) {
            if ($sku !== '') {
                try { setVariantSku3($client, $oldId, $vid, $sku); } catch (Throwable $restoreError) {
                    echo 'ROLLBACK_SKU_WARNING=' . json_encode(['variant_id'=>$vid,'error'=>$restoreError->getMessage()], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) . PHP_EOL;
                }
            }
        }
    }

    echo 'MIGRATE_ROLLBACK=' . json_encode([
        'error'=>$e->getMessage(),
